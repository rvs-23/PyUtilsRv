...
from utilities.viz_utils.generic_plots import hist_distribution, plot_line_chart
...
def test_raw_numeric_plot(monkeypatch):
    ...
    plt_close = plt_close = fig.clf(); plt_close
...
def test_datetime_exact_format(monkeypatch):
    ...
    plt_close = fig.clf(); plt_close


def test_mixed_format_no_resample():
    # Mixed x values: some parseable, some not
    df = pd.DataFrame({
        "x_mixed": ["2025-06-01", "foo", "2025-06-02", "bar", "2025-06-03"],
        "y": [1, 2, 3, 4, 5]
    })
    fig, ax = plot_line_chart(
        df,
    ...
    assert len(lines) == 1
    x_data = lines[0].get_xdata()
    # Should have length 5 and dtype object (mixed)
    assert len(x_data) == 5
    # Verify that parseable strings were converted, and unparseable stayed strings
    converted = [isinstance(val, (pd.Timestamp, datetime)) or isinstance(val, str) for val in x_data]
    assert all(converted)
    plt_close = fig.clf(); plt_close
...
def test_mixed_format_with_resample():
    ...
    df = pd.DataFrame({
        "x_mixed": ["2025-01-01", "foo", "2025-01-01", "bar", "2025-01-02"],
        "val": [10, 20, 30, 40, 50]
    })
...
def test_binning_numeric():
    ...
    np.testing.assert_allclose(x_data, expected_centers, rtol=1e-5)
    np.testing.assert_allclose(y_data, expected_means, rtol=1e-5)
    plt_close = fig.clf(); plt_close
